class_name PomberoStateMachine
extends Node

@export var character : CharacterBody2D
@export var current_state : PomberoState
@export var anim_tree : AnimationTree

var states : Array[PomberoState]

func _ready():
	for child in get_children():
		if child is PomberoState:
			states.append(child)
...
func switch_state(new_state : PomberoState):
...
func on_state_interrupt_state(new_state : PomberoState):
